...
export class HouseResolver {
  ...
  async reviewCount(
    @Root() house: House,
    @Ctx()
    {reviewLoader}: MyContext,
  ) {
    const review =
      await reviewLoader.load(
        house.id,
      );
        ...
        errors: [
          {
            field: 'file',
            message:
              'Загрузите хотя бы 1 изображение',
          },
        ],
      };
    }
